...
module.exports = {
  ...
  injectThemes: function (addBase, config, themes) {
    ...
    // add custom themes
    if (Array.isArray(config("twui.themes"))) {
      config("twui.themes").forEach((item, index) => {
    ...
    let themeOrder = []
    if (Array.isArray(config("twui.themes"))) {
      config("twui.themes").forEach((theme, index) => {
    ...
    } else if (config("twui.themes") != false) {
      themeOrder = ["light", "dark", "cyberpunk", "lofi"]
    } else if (config("twui.themes") == false) {
    ...
    themeOrder.forEach((themeName, index) => {
      if (index === 0) {
        ...
        // auto dark
        if (config("twui.darkTheme")) {
          if (themeOrder[0] != config("twui.darkTheme") && themeOrder.includes(config("twui.darkTheme"))) {
            addBase({
              ["@media (prefers-color-scheme: dark)"]: {
                [":root"]: includedThemesObj[`[data-theme=${config("twui.darkTheme")}]`],
              },
            })
          }
        } else {
